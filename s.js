...
function Rysuj_Krzywe_Sierpinskiego(kolor, stopien) {
	...
	setTimeout(()=>{
		...
		A(stopien); c.beginPath(); c.moveTo(x, y); x += h; y -= h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke();
		B(stopien); c.beginPath(); c.moveTo(x, y); x -= h; y -= h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke();
		C(stopien); c.beginPath(); c.moveTo(x, y); x -= h; y += h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); 
		D(stopien); c.beginPath(); c.moveTo(x, y); x += h; y += h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke();

		function A(i) {
			if (i == 0) return;
			A(i - 1); c.beginPath(); c.moveTo(x, y); x += h; y -= h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			B(i - 1); c.beginPath(); c.moveTo(x, y); x += 2 * h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			D(i - 1); c.beginPath(); c.moveTo(x, y); x += h; y += h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			A(i - 1);
		}
		function B(i) {
			if (i == 0) return;
			B(i - 1); c.beginPath(); c.moveTo(x, y); x -= h; y -= h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			C(i - 1); c.beginPath(); c.moveTo(x, y); y -= 2 * h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			A(i - 1); c.beginPath(); c.moveTo(x, y); x += h; y -= h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			B(i - 1);
		}
		function C(i) {
			if (i == 0) return;
			C(i - 1); c.beginPath(); c.moveTo(x, y); x -= h; y += h; c.lineTo(x, y);  c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			D(i - 1); c.beginPath(); c.moveTo(x, y); x -= 2 * h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			B(i - 1); c.beginPath(); c.moveTo(x, y); x -= h; y -= h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			C(i - 1);
		}
		function D(i) {
			if (i == 0) return;
			D(i - 1); c.beginPath(); c.moveTo(x, y); x += h; y += h; c.lineTo(x, y);c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			A(i - 1); c.beginPath(); c.moveTo(x, y); y += 2 * h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			C(i - 1); c.beginPath(); c.moveTo(x, y); x -= h; y += h; c.lineTo(x, y); c.strokeStyle = cls[i]; c.stroke(); c.closePath();
			D(i - 1);
		}
	})
